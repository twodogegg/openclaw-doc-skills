...
# Change primary model (recommended when using built-in providers)
jq '.agents.defaults.model.primary = "anthropic/claude-sonnet-4"' ~/.openclaw/openclaw.json > /tmp/oc.json && mv /tmp/oc.json ~/.openclaw/openclaw.json

# Optional model allowlist/aliases.
# NOTE: if agents.defaults.models is set, models outside this list may be blocked.
jq '.agents.defaults.models = {
...
# Add a custom OpenAI-compatible provider and select it as primary
jq '.models.mode = "merge" |
    ...
    .agents.defaults.model.primary = "custom-proxy/llama-3.1-8b" |
    .agents.defaults.models["custom-proxy/llama-3.1-8b"] = {"alias": "llama"}' \
~/.openclaw/openclaw.json > /tmp/oc.json && mv /tmp/oc.json ~/.openclaw/openclaw.json
...
### Multi-agent + Multi-Feishu accounts

Use `agents.list` to define agents, `channels.feishu.accounts` to define Feishu bot accounts, and `bindings` to route each Feishu account to a specific agent.

```bash
# 1) Add two agents (if missing)
jq '
  if any(.agents.list[]?; .id=="writer") then . else
    .agents.list += [{
      "id":"writer",
      "name":"writer",
  ...
  | if any(.agents.list[]?; .id=="ops") then . else
    .agents.list += [{
      "id":"ops",
      "name":"ops",
...
# 2) Add two Feishu accounts under channels.feishu.accounts
jq '
  .channels.feishu.accounts.writer = {
    "appId":"${OPENCLAW_FEISHU_WRITER_APP_ID}",
    "appSecret":"${OPENCLAW_FEISHU_WRITER_APP_SECRET}",
    "botName":"Writer Bot",
    "requireMention": true
  }
  | .channels.feishu.accounts.ops = {
    "appId":"${OPENCLAW_FEISHU_OPS_APP_ID}",
    "appSecret":"${OPENCLAW_FEISHU_OPS_APP_SECRET}",
    "botName":"Ops Bot",
    "requireMention": true
  }
' ~/.openclaw/openclaw.json > /tmp/oc.json && mv /tmp/oc.json ~/.openclaw/openclaw.json

# 3) Bind Feishu account -> agent
jq '
  if any(.bindings[]?; .agentId=="writer" and .match.channel=="feishu" and .match.accountId=="writer") then . else
    .bindings += [{"agentId":"writer","match":{"channel":"feishu","accountId":"writer"}}]
  end
  | if any(.bindings[]?; .agentId=="ops" and .match.channel=="feishu" and .match.accountId=="ops") then . else
    .bindings += [{"agentId":"ops","match":{"channel":"feishu","accountId":"ops"}}]
  end
' ~/.openclaw/openclaw.json > /tmp/oc.json && mv /tmp/oc.json ~/.openclaw/openclaw.json
...
```bash
# See all agents
jq -r '.agents.list[] | .id' ~/.openclaw/openclaw.json

# See all Feishu accounts
jq -r '.channels.feishu.accounts | keys[]' ~/.openclaw/openclaw.json

# See Feishu bindings
jq -r '.bindings[] | select(.match.channel=="feishu") | "\(.agentId) <= feishu:\(.match.accountId)"' ~/.openclaw/openclaw.json
```
